    ...
    >
      ...
      <div className="relative p-4 sm:p-6 md:p-8">
        ...
        >
          ...
          >
            <GraduationCap className="h-6 w-6 sm:h-7 sm:w-7 md:h-8 md:w-8 text-white drop-shadow-lg" />
          </motion.div>
          <div>
            <h3 className="text-lg sm:text-xl md:text-2xl font-bold bg-gradient-to-r from-emerald-600 to-teal-600 dark:from-emerald-400 dark:to-teal-400 bg-clip-text text-transparent">
              Educational Journey
            </h3>
        ...
        <div className="relative">
          ...
          {profile.educationalQualifications && profile.educationalQualifications.length > 0 && (
            <div className="absolute left-3 sm:left-4 md:left-6 top-0 bottom-0 w-0.5 bg-gradient-to-b from-emerald-200 via-teal-200 to-cyan-200 dark:from-emerald-700 dark:via-teal-700 dark:to-cyan-700"></div>
          )}

          {profile.educationalQualifications && profile.educationalQualifications.length > 0 ? (
            <div className="space-y-4 sm:space-y-6">
              {profile.educationalQualifications.length > 2 ? (
                <div className="max-h-[400px] sm:max-h-[500px] overflow-y-auto scrollbar-hide">
                  {profile.educationalQualifications.map((education, index) => (
                    <motion.div
                      ...
                      whileHover={{ y: -2, scale: 1.02 }}
                      className="group/item relative flex items-start gap-3 sm:gap-4 md:gap-6 mb-4 sm:mb-6"
                    >
                        ...
                        <div className="space-y-3 sm:space-y-4">
                          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2 sm:gap-3">
                            <div className="space-y-1.5 sm:space-y-2">
                              ...
                              <div className="flex flex-wrap gap-2 sm:gap-4 text-xs sm:text-sm text-emerald-600 dark:text-emerald-400">
                              ...
                              {education.description && (
                                <p className="text-xs sm:text-sm text-emerald-600 dark:text-emerald-400 italic bg-emerald-50/50 dark:bg-emerald-900/20 p-2 sm:p-3 rounded-xl border border-emerald-200/30 dark:border-emerald-700/30">
                                  "{education.description}"
                                </p>
                              )}
                            </div>
                          ...
                          {education.achievements && education.achievements.length > 0 && (
                            <div className="pt-2 border-t border-emerald-200/30 dark:border-emerald-700/30">
                              <p className="text-xs sm:text-sm font-semibold text-emerald-700 dark:text-emerald-300 mb-2 sm:mb-3 flex items-center gap-1.5 sm:gap-2">
                              ...
                              <div className="flex flex-wrap gap-1.5 sm:gap-2">
                                {education.achievements.map((achievement, i) => (
                                  <motion.span
                                    ...
                                    className="px-2 sm:px-3 py-1 bg-gradient-to-r from-amber-100/80 to-orange-100/80 dark:from-amber-900/30 dark:to-orange-900/30 text-amber-700 dark:text-amber-300 rounded-full text-[10px] sm:text-xs font-medium border border-amber-200/50 dark:border-amber-700/50 backdrop-blur-sm"
                                  >
                                    {achievement}
                                  </motion.span>
                                ))}
                ...
                profile.educationalQualifications.map((education, index) => (
                  <motion.div
                    ...
                    transition={{ delay: 0.3 + index * 0.1, duration: 0.5 }}
                    whileHover={{ y: -2, scale: 1.02 }}
                    className="group/item relative flex items-start gap-3 sm:gap-4 md:gap-6"
                  >
                    {/* Enhanced Timeline Node */}
                    <motion.div
                      className={`relative z-10 w-10 h-10 sm:w-12 sm:h-12 rounded-full border-4 border-white/90 dark:border-slate-800/90 shadow-lg flex items-center justify-center transition-all duration-300 ${
            ...
            <div className="w-full">
              ...
              >
                <motion.div
                  ...
                  transition={{ duration: 4, repeat: Infinity, ease: "easeInOut" }}
                >
                  <GraduationCap className="h-6 w-6 sm:h-7 sm:w-7 md:h-8 md:w-8 text-slate-400 dark:text-slate-500" />
                </motion.div>
                <h4 className="text-sm sm:text-base text-slate-900 dark:text-slate-100 font-semibold mb-1 sm:mb-2">No Education Yet</h4>
                <p className="text-xs sm:text-sm text-slate-600 dark:text-slate-400">Add your educational qualifications to showcase your academic journey!</p>
              </motion.div>
            </div>
          )}
        </div>
