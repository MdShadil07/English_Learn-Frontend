...
import DetailedPronunciationAnalysis from './DetailedPronunciationAnalysis';
...
const getAnalysisTip = (analysis: any) => {
  ...
  // Use MFA deep analysis component scores if available
  if (analysis.componentScores) {
    if (analysis.componentScores.vowelQuality < 80) return `Vowel distortion detected. Make sure to shape your vowels accurately for "${targetWord}".`;
    if (analysis.componentScores.consonantCompletion < 80) return `Consonant drop or distortion detected. Enunciate the consonants clearly in "${targetWord}".`;
    if (analysis.componentScores.durationTiming < 70) return `The pacing for "${targetWord}" was off. Hold the sound for the correct duration.`;
  }
...
const ArticulationMouth = ({ phonemes = [], animStep = 0 }) => {
  // Simple mapping to change mouth shape slightly based on phoneme type
  const currentPhoneme = phonemes.length > 0 ? phonemes[animStep % phonemes.length] : '';
  const isVowel = /^[AEIOU]/.test(currentPhoneme || '');
  const isBilabial = /^(P|B|M|W)/.test(currentPhoneme || '');
  
  // Normal state
  let topLip = "M15,50 Q30,35 50,40 Q70,35 85,50 Q50,45 15,50 Z";
  let bottomLip = "M15,50 Q50,55 85,50 Q70,75 50,75 Q30,75 15,50 Z";
  let teethGap = "M30,48 L70,48 L65,52 L35,52 Z";

  if (isVowel) {
    // Open mouth
    topLip = "M20,45 Q35,25 50,30 Q65,25 80,45 Q50,40 20,45 Z";
    bottomLip = "M20,45 Q50,65 80,45 Q65,85 50,85 Q35,85 20,45 Z";
    teethGap = "M35,45 L65,45 L60,49 L40,49 Z";
  } else if (isBilabial) {
    // Closed mouth (lips pressed)
    topLip = "M10,50 Q30,45 50,50 Q70,45 90,50 Q50,48 10,50 Z";
    bottomLip = "M10,50 Q50,52 90,50 Q70,60 50,60 Q30,60 10,50 Z";
  }

  return (
    <div className="w-24 h-24 relative flex items-center justify-center flex-col">
      <svg viewBox="0 0 100 100" className="w-full h-full drop-shadow-[0_4px_10px_rgba(244,63,94,0.4)] transition-all duration-300">
        <path d="M10,50 Q50,20 90,50 Q50,80 10,50 Z" fill="#f43f5e" opacity="0.15" filter="blur(4px)" />
        <path d={topLip} fill="url(#lipGradient)" className="transition-all duration-300" />
        <path d={bottomLip} fill="url(#lipGradient)" className="transition-all duration-300" />
        {/* Teeth */}
        <path d={teethGap} fill="#ffffff" opacity="0.9" className="transition-all duration-300" />
        <defs>
      ...
      {currentPhoneme && (
        <div className="absolute -bottom-2 bg-indigo-600 text-white text-[10px] font-bold px-2 py-0.5 rounded-full uppercase tracking-wider">
          /{currentPhoneme}/
        </div>
      )}
    </div>
...
export default function App({ isOpen = true, onClose = () => { } }) {
  ...
  const [speechProfile, setSpeechProfile] = useState<any>(null);
  const [showDetailedAnalysis, setShowDetailedAnalysis] = useState(false);

  useEffect(() => {
    if (sessionState === 'results') {
      pronunciationService.getCoachProfile().then(res => {
        if (res?.data?.profile) setSpeechProfile(res.data.profile);
      }).catch(console.error);
    }
  }, [sessionState]);
  ...
  const submitPracticeAttempt = async (recording: any, metadataOverrides: any = {}) => {
  ...
  return (
    <AnimatePresence>
            ...
            >
              ...
              {sessionState === 'analyzing' ? (
                ...
                <PronunciationRecorder
                  ...
                  onSubmitRecording={(recording, opts) => submitPracticeAttempt(recording, opts)}
                    ...
                    <div className="flex-1 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                      ...
                      {!sessionInfo && (
                        <button
                          ...
                          className="self-start sm:self-auto px-4 py-1.5 bg-rose-600 hover:bg-rose-700 active:scale-95 text-white rounded-xl text-xs font-bold transition-all shrink-0 shadow-sm"
                        >
                          Retry Loading
                        </button>
                      )}
                    </div>
                    <button onClick={() => setErrorMessage('')} className="text-rose-400 hover:text-rose-600 dark:hover:text-rose-300 shrink-0">
                      <X className="w-4 h-4" />
                    </button>
                  </motion.div>
                )}
                  ...
                  <div className="xl:col-span-3 flex flex-col gap-6">
                    ...
                    <div className="bg-white dark:bg-[#131722] rounded-2xl p-4 border border-slate-200 dark:border-slate-800 shadow-sm flex items-center justify-between">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <h3 className="text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider mb-0.5">Analysis Engine</h3>
                          <p className="text-sm font-bold text-slate-800 dark:text-slate-200">
                            {attemptResult?.processingMetrics?.alignment?.timing_source === 'mfa' || attemptResult?.metadata?.timing_source === 'mfa' ? 'Premium In-Depth (MFA)' : 'Standard Fast-Path'}
                          </p>
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="bg-white dark:bg-[#131722] rounded-3xl p-6 border border-slate-200 dark:border-slate-800 shadow-sm relative overflow-hidden">
                      ...
                      {speechProfile?.overallScore !== undefined && (
                        <div className="mt-4 pt-4 border-t border-slate-100 dark:border-slate-800 relative z-10">
                          <div className="flex justify-between items-center">
                            <div className="flex items-center gap-2">
                              <Activity className="w-4 h-4 text-indigo-500" />
                              <span className="text-xs font-bold text-slate-600 dark:text-slate-300">Historical Average</span>
                            </div>
                            <span className="text-sm font-extrabold text-slate-900 dark:text-white">{speechProfile.overallScore}%</span>
                          </div>
                          <p className="text-[10px] font-medium text-slate-500 dark:text-slate-400 mt-1.5 leading-relaxed">
                            {overallScore >= speechProfile.overallScore ? "Trending Upwards! Your current attempt beat your historical baseline." : "Slightly below your baseline. Keep practicing to maintain consistency!"}
                          </p>
                        </div>
                      )}
                    </div>

                    {(attemptResult?.processingMetrics?.scoring?.alignmentConfidence !== undefined ? attemptResult.processingMetrics.scoring.alignmentConfidence : attemptResult?.metadata?.alignmentConfidence) !== undefined && (attemptResult?.processingMetrics?.scoring?.alignmentConfidence !== undefined ? attemptResult.processingMetrics.scoring.alignmentConfidence : attemptResult?.metadata?.alignmentConfidence) < 0.40 && (
                      <div className="bg-rose-50 dark:bg-rose-500/10 rounded-3xl p-5 border border-rose-100 dark:border-rose-500/20 shadow-sm">
                        ...
                        <div className="text-xs font-bold text-rose-600 dark:text-rose-400 mb-2">
                          Low ({Math.round((attemptResult?.processingMetrics?.scoring?.alignmentConfidence !== undefined ? attemptResult.processingMetrics.scoring.alignmentConfidence : attemptResult?.metadata?.alignmentConfidence) * 100)}%)
                        </div>
                    ...
                    {attemptResult?.phenomena && attemptResult.phenomena.length > 0 && (
                      <div className="bg-white dark:bg-[#131722] rounded-3xl p-6 border border-slate-200 dark:border-slate-800 shadow-sm relative overflow-hidden">
                        ...
                        <div className="space-y-3 relative z-10">
                          {attemptResult.phenomena.slice(0, 3).map((pattern: any, i: number) => {
                            const isCritical = pattern.id === 'th_substitution' || pattern.id === 'vowel_neutralization';
                            return (
                              <div key={i} className={`flex items-start gap-3 p-3 rounded-xl border ${isCritical ? 'bg-rose-50 dark:bg-rose-500/5 border-rose-100 dark:border-rose-500/10' : 'bg-slate-50 dark:bg-slate-800/30 border-slate-200 dark:border-slate-700/50'}`}>
                                <div className={`w-6 h-6 rounded-full flex items-center justify-center shrink-0 mt-0.5 ${isCritical ? 'bg-rose-100 dark:bg-rose-500/20' : 'bg-slate-200 dark:bg-slate-700'}`}>
                                  <AlertTriangle className={`w-3 h-3 ${isCritical ? 'text-rose-500' : 'text-slate-500'}`} />
                                </div>
                                <div>
                                  <h4 className="text-xs font-bold text-slate-800 dark:text-slate-200 capitalize">{pattern.name.replace(/_/g, ' ')}</h4>
                                  <p className="text-[10px] text-slate-600 dark:text-slate-400 mt-1 leading-relaxed">
                                    {pattern.evidence?.[0] || "Recurring phonetic pattern detected in your speech."}
                                  </p>
                                  {pattern.drills?.[0] && (
                                    <div className="mt-2 text-[10px] font-medium text-indigo-600 dark:text-indigo-400 flex items-center gap-1.5">
                                      <Sparkles className="w-3 h-3" /> {pattern.drills[0].instruction}
                                    </div>
                                  )}
                                </div>
                              </div>
                    ...
                    {/* AI Communication Coach Integration */}
                    <div className="mt-2">
                          ...
                          <div className="bg-slate-50 dark:bg-[#0f1219] rounded-3xl p-6 md:p-8 border border-slate-200 dark:border-slate-800/80 shadow-inner relative">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                              ...
                              <div className="bg-white dark:bg-[#131722] rounded-2xl border border-slate-200 dark:border-slate-800 p-6 shadow-sm relative overflow-hidden flex flex-col justify-between min-h-[250px]">
                                ...
                                <div className="flex flex-col gap-6 relative z-10 mb-4">
                                  {/* Articulation Guide Content */}
                                  <div className="flex items-center gap-6">
                                    <ArticulationMouth phonemes={activeWord.ipa?.split(' ') || []} animStep={animStep} />
                                    <div className="flex-1 bg-slate-50 dark:bg-[#1A1F2B]/80 rounded-xl p-4 border border-slate-100 dark:border-slate-700/50 backdrop-blur-sm shadow-[0_4px_20px_rgba(0,0,0,0.02)]">
                                      <div className="flex items-center gap-2 mb-1.5">
                                        <p className="text-2xl font-mono font-bold text-indigo-500 dark:text-indigo-400">
                                          {activeWord.ipa ? `/${activeWord.ipa}/` : activeWord.targetSound}
                                        </p>
                                        {activeWord.status !== 'perfect' && (
                                  ...
                                  {/* Speech DNA Word-Level Match */}
                                  {attemptResult?.phenomena && (() => {
                                    const wordPhenomena = attemptResult.phenomena.filter((p: any) => p.affectedWords?.includes(activeWord.word));
                                    if (wordPhenomena.length === 0) return null;
                                    return (
                                      <div className="bg-rose-50 dark:bg-rose-500/10 rounded-xl p-3 border border-rose-100 dark:border-rose-500/20">
                                        <h6 className="text-[10px] font-bold text-rose-500 uppercase tracking-wider mb-2">Speech DNA Match</h6>
                                        <div className="space-y-2">
                                          {wordPhenomena.map((pattern: any, idx: number) => (
                                            <div key={idx} className="flex gap-2 items-start">
                                              <AlertTriangle className="w-3 h-3 text-rose-500 mt-0.5 shrink-0" />
                                              <div>
                                                <p className="text-xs font-bold text-slate-800 dark:text-slate-200 capitalize">{pattern.name.replace(/_/g, ' ')}</p>
                                                {pattern.drills?.[0] && (
                                                  <p className="text-[10px] font-medium text-slate-600 dark:text-slate-400 mt-0.5">{pattern.drills[0].instruction}</p>
                                                )}
                                              </div>
                                            </div>
                                          ))}
                                ...
                                <div className="relative z-10 mt-auto pt-4 border-t border-slate-100 dark:border-slate-800">
                                  <div className="flex gap-1.5 items-center justify-center mb-2">
                                    {[...Array(15)].map((_, i) => (
                                      <div key={i} className={`h-2 rounded-full transition-all duration-300 ${i < animStep
                                          ? activeWord.status === 'perfect'
      ...
      <AnimatePresence>
        {showDetailedAnalysis && attemptResult && (
